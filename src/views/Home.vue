<template>
  <div class="w-full flex flex-col">
    ...
    <div  class="w-full flex flex-wrap border-2 border-green-200 py-4 justify-evenly">
      ...
      <!-- Seccion -->
      <div v-for="(datos, a) in arrayData" :key="a" class="flex flex-wrap justify-start items-start flex-none mb-4 border-4 border-red-900 w-full">
    <div v-for="(datos2, b) in arrayData[a]" :key="b" class="flex flex-col flex-wrap md:justify-center md:items-center sm:justify-start sm:items-start sm:w-full md:w-1/2 bg-green-400 border-4 border-green-800">
      <!-- COMPONENTE SECCION -->
      <div class="flex justify-center items-center bg-red-300">
        <!-- Icono seccion -->
        <div class="w-16 h-16 rounded-md shadow flex justify-center items-center mr-4 flex-none" :class="datos2.seccion.toLowerCase()">
          <h1 class="truncate font-semibold">{{datos2.seccion}}</h1>
        </div>
        <!-- Icono seccion -->

        <!-- Info de la seccion -->
        <div class="flex justify-center items-center rounded-2xl bg-white shadow flex-none">    
            <div class="flex flex-col items-center justify-center p-4">
              <img class="w-4" src="..\assets\incidencias.svg" alt="">
              <h1 class="font-semibold text-2xl mt-2 truncate">{{234}}</h1>
